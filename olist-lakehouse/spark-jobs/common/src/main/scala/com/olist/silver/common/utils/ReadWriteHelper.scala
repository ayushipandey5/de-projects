...
object ReadWriteHelper {
  ...
  def readFromSource(sourcePath : String,
    ...
    val lastProcessedDate = if(checkPointVal.length >= 10)  checkPointVal.substring(0,9) else "1900-01-01"
  ...
  private def getCheckPointValue(checkPointPath : String)(implicit sparkSession: SparkSession): Option[String] = {
    try {
      ...
      else Some(checkPointVal.collect()(0).getString(0))
    } catch {
      case e: Exception =>
